      ...
      >
        {items.map((item) => (
          <motion.div
            ...
            variants={fadeInUp}
            whileHover={{ y: -8, scale: 1.02, boxShadow: '0 12px 30px rgba(0,0,0,0.12)' }}
          ...
          >
            ...
            <div className="p-5">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-[var(--primary-green)] font-bold">${item.price.toFixed(2)}</span>
              </div>
              <p className="text-[var(--text-secondary)] text-sm mb-4">{item.description}</p>
              <div className="flex flex-wrap gap-2 mb-4">
                {item.popular && (
                  <motion.span 
                    className="bg-primary-red text-white text-xs px-2 py-1 rounded-full"
